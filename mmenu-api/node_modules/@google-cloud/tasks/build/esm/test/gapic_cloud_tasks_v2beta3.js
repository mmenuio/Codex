...
describe('v2beta3.CloudTasksClient', () => {
    ...
    describe('bufferTask', () => {
        it('invokes bufferTask without error', async () => {
            ...
            client.initialize();
            const request = generateSampleMessage(new protos.google.cloud.tasks.v2beta3.BufferTaskRequest());
            const defaultValue1 = getTypeDefaultValue('.google.cloud.tasks.v2beta3.BufferTaskRequest', ['queue']);
            request.queue = defaultValue1;
            const defaultValue2 = getTypeDefaultValue('.google.cloud.tasks.v2beta3.BufferTaskRequest', ['taskId']);
            request.taskId = defaultValue2;
            const expectedHeaderRequestParams = `queue=${defaultValue1 !== null && defaultValue1 !== void 0 ? defaultValue1 : ''}&task_id=${defaultValue2 !== null && defaultValue2 !== void 0 ? defaultValue2 : ''}`;
            const expectedResponse = generateSampleMessage(new protos.google.cloud.tasks.v2beta3.BufferTaskResponse());
            client.innerApiCalls.bufferTask = stubSimpleCall(expectedResponse);
            const [response] = await client.bufferTask(request);
            assert.deepStrictEqual(response, expectedResponse);
            const actualRequest = client.innerApiCalls.bufferTask.getCall(0).args[0];
            assert.deepStrictEqual(actualRequest, request);
            const actualHeaderRequestParams = client.innerApiCalls.bufferTask.getCall(0).args[1].otherArgs.headers['x-goog-request-params'];
            assert(actualHeaderRequestParams.includes(expectedHeaderRequestParams));
        });
        it('invokes bufferTask without error using callback', async () => {
            ...
            client.innerApiCalls.bufferTask =
                stubSimpleCallWithCallback(expectedResponse);
            const promise = new Promise((resolve, reject) => {
                client.bufferTask(request, (err, result) => {
                    if (err) {
                        reject(err);
                    }
                    else {
